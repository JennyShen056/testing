...
# parser.add_argument("--rm1", type=str)
# parser.add_argument("--rm2", type=str)
# parser.add_argument("--rm1_gpu", type=str, default="cuda:1")
...
for run_config in run_configs:
    if "rm_weight" not in run_config:
        ...
        exit(1)
# for run_config in run_configs:
#     if "rm1_weight" not in run_config or "rm2_weight" not in run_config:
#         print(f"Missing keys 'rm1_weight' or 'rm2_weight' in {run_config=}")
#         exit(1)
#     elif "topk" not in run_config:
#         print(f"Missing key 'topk' in {run_config=}")
#         exit(1)
...
print(f"[INFO]: Done")
# print(f"[INFO]: Loading models ({args.llm=}, {args.rm1=}, {args.rm2=})")
# search = ARGS(
#     llm_path=args.llm,
#     rm1_path=args.rm1,
#     rm2_path=args.rm2,
#     llm_dev=args.llm_gpu,
#     rm1_dev=args.rm1_gpu,
#     rm2_dev=args.rm2_gpu
# )
# print(f"[INFO]: Done")
...
# def runprompt(prompt: str, rm1_weight=0., rm2_weight=0., topk=5, new_token=24, mode="p_sigmoid_mixing", sample_temp=None, llm_dev:str="cuda:0") -> str:
#     tokens = search.generate(
#         prompt,
#         method=mode,
#         topk=topk,
...
for config_num, run_config in enumerate(run_configs):
    ...
    #         current_prompt,
    #         float(run_config["rm1_weight"]),
    #         float(run_config["rm2_weight"]),
    #         run_config["topk"],
    #         args.max_new_token,
    #         run_config["mode"],
