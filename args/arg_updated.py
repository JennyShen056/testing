...
class ARGS:
    ...
    def generate_greedy_step_large(self, mout, input_ids, pre_screen_beam_width=40, weight=0., rm_cached=None, chunk_size=10, debug=True, _use_cache=True):
        ...
        for chunk, chunk_logits in zip(even_chunk(flat_trme.to(self.rm_dev), chunk_size), even_chunk(prescreen_logits.flatten(), chunk_size)):
            ...
            chunk_text = self.rm_tokenizer.batch_decode(chunk, skip_special_tokens=True)
            wrapped_chunk = [self.rm_tokenizer.apply_chat_template([{"role": "user", "content": text}], tokenize=False) for text in chunk_text]
            rm_inputs = self.rm_tokenizer(wrapped_chunk, return_tensors="pt", padding=True, truncation=True).to(self.rm_dev)
    ...
    def generate_step(self, mout, input_ids, pre_screen_beam_width=40, weight=0., method="greedy", temperature=0.7, rm_cached=None, debug=True):
        ...
        return flat_trme[top_k_ids], None  # `None` because `rm_cached` is not used by the RM
    ...
    def generate(self, prompt, weight=0., topk=1, max_new_token=128, method="greedy", temperature=0.7, chunk_size=5, debug=False):
        ...
        rm_cached = None
        cached = None  # Initialize cached for past_key_values

        for _ in range(max_new_token):
            with torch.no_grad():
                # Ensure proper handling of cached
                if cached is None:
                    mout = self.LLM(
                        input_ids=tokens,
                    ...
                    mout = self.LLM(
                        input_ids=tokens[:, -1:],  # Only pass the last token
                        attention_mask=torch.ones((tokens.shape[0], 1), device=self.llm_dev),  # Mask for the last token
