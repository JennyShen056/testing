...
def create_attention_mask(seq_len, bsz=1, device="cpu"):
    return torch.ones((bsz, seq_len), device=device)
...
class ARGS:
    ...
    def generate_greedy_step_large(self, mout, input_ids, pre_screen_beam_width=40, weight=0., rm_cached=None, chunk_size=10, debug=True, _use_cache=True):
        ...
        to_rm_eval = torch.dstack((expanded_tis, prescreen_tokens)).to(self.llm_dev)
        if debug: print(f"{to_rm_eval.shape=}")

        if debug: print(f"{out_logits.shape[0] * pre_screen_beam_width=}")
        flat_trme = to_rm_eval.view(out_logits.shape[0] * pre_screen_beam_width, -1).to(self.llm_dev)
        ...
        for chunk, chunk_logits in zip(even_chunk(flat_trme.to(self.rm_dev), chunk_size), even_chunk(prescreen_logits.flatten().to(self.rm_dev), chunk_size)):
            pkv = None if not _use_cache else rm_cached

            rm_out = self.RM(
                **self.LLM.prepare_inputs_for_generation(
                    input_ids=chunk,
                    attention_mask=create_attention_mask(chunk.shape[1], chunk.shape[0], device=self.rm_dev),
                    past_key_values=pkv,
                    use_cache=True
                )
            )
    ...
    def generate_step(self, mout, input_ids, pre_screen_beam_width=40, weight=0., method="greedy", temperature=0.7, rm_cached=None, debug=True):
        ...
        if rm_cached is None:
            rm_out = self.RM(
                **self.LLM.prepare_inputs_for_generation(
                    ...
                    attention_mask=create_attention_mask(flat_trme.shape[1], flat_trme.shape[0], device=self.rm_dev),
        ...
        new_scores = rewards * weight + prescreen_logits.flatten().to(self.llm_dev)
        if debug: print(f"{new_scores.shape=}")
        ...
        for _ in iterator_obj:
            ...
            with torch.no_grad():
                if cached is None:
                    mout = self.LLM(
                        **self.LLM.prepare_inputs_for_generation(
                            input_ids=tokens.to(self.llm_dev),
                            attention_mask=create_attention_mask(tokens.shape[1], tokens.shape[0], device=self.llm_dev),
                            past_key_values=None,
                            use_cache=True
                        )
                    )
